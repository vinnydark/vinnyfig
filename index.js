...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `saiu @${num.split('@')[0]}filho da putakkkkkkkkkkkkkkkkkkkkkkk`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬ AGUARDE ❭ _caso nao faça,tente de novo...⏳_',
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'essa e a lista de números bloqueados:\n'
				...
				case 'ocr':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('qual a foto?/só funciona em fotos')
					}
					break
				case 'stiker':
				case 'sticker':
				case 's':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`mande uma foto com a legenda ${prefix}sticker ou marque uma`)
					}
					break
			case 'dono':
                ...
                client.sendMessage(from, 'sucesso, descrição do grupo modificada', text, {quoted: mek})
                break
				case 'gtts':
				case 'tts':
					...
					rano = getRandom('.ogg')
					dtt.length > 800
					? reply('tamaluco fi muito grande vo faze naokkkkkkkkkkkkkkkkkkkkkkkkkkkkkk')
	            ...
	            case 'setpp':
                    ...
                    reply('sucesso, imagem do grupo modificada')
                    break
                case 'apakah':
					apakah = body.slice(1)
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
                ...
                case 'speed':
                    ...
                    client.sendMessage(from, `ping: ${latensi.toFixed(4)} _segundos 😳_`, text, { quoted: mek})
                    break
                case 'tagme':
					var nom = mek.participant
					const tag = {
					text: `@${nom.split("@s.whatsapp.net")[0]} marcado!`,
                ...
                case 'report':
                     const pesan = body.slice(8)
                      if (pesan.length > 300) return client.sendMessage(from, 'reporte alguém ou mande uma mensagem para o dono', msgType.text, {quoted: mek})
                    ...
                    client.sendMessage('5511954245328@s.whatsapp.net', options, text, {quoted: mek})
                ...
                case 'request':
                    ...
                    client.sendMessage('11954245328@s.whatsapp.net', options, text, {quoted: mek})
				...
				case 'memeindo':
					...
					break
				case 'ssweb':
					if (args.length < 1) return reply('qual o url?')
					teks = body.slice(7)
					reply(mess.wait)
               ...
               case 'infogempa':
				...
				break
					...
					if (args[0] === 'abrir') {
						...
						reply(` *comando recebido, apenas administradores podem enviar mensagens* `)
			    ...
			    case 'otagall':
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  BOT DO PACOCA 〙', members_id, true)
					break
				case 'clearall':
					...
					reply('todos os chats deletados com sucesso :)')
					break
				case 'bc':
					if (!isOwner) return reply('Kamu siapa?')
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *ISSO É UMA TRANSMISSÂO* ]\n\n${body.slice(4)}`)
						}
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('o número é privado, falha ao adicionar')
					}
					break
			    case 'kick':
					if (!isGroup) return reply(mess.only.group)
					...
					if (mentioned.length > 1) {
						teks = 'KKKKKKKKKKKKKKKKKKKKzuo pra carai:\n'
						...
						mentions(`KKKKKKKKKKKKKKKKKKKKzuo pra carai: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'edotense':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = 'agora o:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]} é admin\n`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`agora o: @${mentioned[0].split('@')[0]} é admin`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				case 'delete':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ' comando recebido, mensagem excluída:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`comando recebido, mensagem excluída: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.deleteMessage(from, mentioned)
					}
					break
			    case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'foi rebaixado muito burro:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`foi rebaixado muito burro: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'listadmins':
					...
					break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ marque uma figurinha para virar foto (não funciona em animadas) ❌')
					reply(mess.wait)
			    ...
			    case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						reply('ativado com sucesso o modo nsfw neste grupo ✔️')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./src/nsfw.json', JSON.stringify(nsfw))
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('recurso de boas vindas desativado neste grupo ✔️')
					} else {
						reply('1 para ativar, 0 para desativar')
					}
				case 'clone':
		        ...
		        case 'setpp2':
                    ...
                    media = await client.downloadAndSaveMediaMessage(mek)
                    await client.updateProfilePicture (from, media)
                    reply('alterando com sucesso ícone do grupo')
                    break
                case 'bc2':
					if (!isadminbot) return reply('qm e vc mlk')
					if (args.length < 1) return reply('.......')
					anu = await client.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
				...
				case 'hidetag2':
					if (!isGroup) return reply(mess.only.group)
					if (!isadminbot) return reply('qm e vc random')
					var value = body.slice(9)
					var group = await client.groupMetadata(from)
					var member = group['participants']
					var mem = []
				...
				case 'setpp3':
                    ...
                    break
                case 'bc3':
					if (!isfrendsowner) return reply('qm e vc random')
					if (args.length < 1) return reply('.......')
					anu = await client.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
